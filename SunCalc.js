...
// ==================== SunCalc Wrapper Class ====================

class SunCalc {
	constructor(coordinates, date, timezone) {
		this._latitude = coordinates.x;
		this._longitude = coordinates.y;
		this._altitude = coordinates.hasOwnProperty("z") ? coordinates.z : 0;
		this._date = date ? new Date(date.toISOString()) : new Date();
		this._timezone = timezone !== undefined ? Math.floor(timezone) : -this._date.getTimezoneOffset() / 60;
		this._sunPosition = new Vec2(0, 0);
	...
	setLocation(coordinates, timezone) {
		...
		this._longitude = coordinates.y;
		this._altitude = coordinates.hasOwnProperty("z") ? coordinates.z : 0;
		if (timezone !== undefined) this._timezone = Math.floor(timezone);
		this._updatePosition();
	}
	...
	_updatePosition() {
		const position = getSolarPosition(this._latitude, this._longitude, this._date, {
			elevation: this._altitude,
			timezone: this._timezone
		});
		if (position) {
			this._sunPosition = new Vec2(position.azimuth, position.elevation);
		}
	}
	...
	getSunrise() {
		return getSunrise(this._latitude, this._longitude, this._date, {
			elevation: this._altitude,
			timezone: this._timezone
		});
	}
	...
	getSunset() {
		return getSunset(this._latitude, this._longitude, this._date, {
			elevation: this._altitude,
			timezone: this._timezone
		});
	}
